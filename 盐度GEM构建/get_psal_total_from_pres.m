function [psal_total] = get_psal_total_from_pres(pres_concrete)
%%
% 首先，指定包含.mat文件的文件夹路径
...
for i = 1:length(file_list)
    ...
    for j = 1:length(data_GOM)
            ...
            if ismember(pres_concrete, data_GOM{1,j}.P_adj_sample_noNaN)
                    % 找出值为 10 的元素索引
                    index = find(data_GOM{1,j}.P_adj_sample_noNaN == pres_concrete);
              
                    % 找出最接近 10 的元素索引
                    %[~,index] = min(abs((data_GOM{1,j}.P_adj_sample_noNaN)- pres_concrete));

                    %获取每个剖面压力为10时的温度数据存进temp_total
                    psal_total = [psal_total,data_GOM{1,j}.S_adj_sample_noNaN(index)];
            else                  
                    psal_total = [psal_total,NaN];       
            end
    end
end
