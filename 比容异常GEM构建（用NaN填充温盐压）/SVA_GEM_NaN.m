...
load('D:\Program Files (x86)\MATLAB\matlab_study\GEM构建\比容异常GEM构建（用NaN填充温盐压）\SVA_total_from_all_resolution_NaN.mat');
load('D:\Program Files (x86)\MATLAB\matlab_study\GEM构建\tau_total_sort.mat');

SVA_all_resolution = zeros(125,4000);

for i = 1:125
    SVA_all_resolution(i,:) = SVA_total_from_all_resolution_NaN(i).bb;
    pres(i) = SVA_total_from_all_resolution_NaN(i).resolution;
end
